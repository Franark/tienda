...
<main>
    <div class="productos">
        <?php foreach ($productos as $p): ?>
            <div class="producto">
                ...
                <a href="?page=verProduct&idProducto=<?php echo $p['idProducto']; ?>">Ver más</a>
            </div>
        <?php endforeach; ?>
    </div>
    ...
    <nav class="pagination">
        <ul>
            <li><a href="?page=catalogoProducto&current_page=<?= max(1, $current_page - 1) ?>&search_query=<?= urlencode($search_query) ?>&categoriaProducto_idCategoriaProducto=<?= urlencode($categoriaProducto_idCategoriaProducto) ?>&marca_idMarca=<?= urlencode($marca_idMarca) ?>">Atrás</a></li>
            <p>Página <?= $current_page ?> de <?= $total_pages ?></p>
            <li><a href="?page=catalogoProducto&current_page=<?= min($current_page + 1, $total_pages) ?>&search_query=<?= urlencode($search_query) ?>&categoriaProducto_idCategoriaProducto=<?= urlencode($categoriaProducto_idCategoriaProducto) ?>&marca_idMarca=<?= urlencode($marca_idMarca) ?>" <?= $current_page >= $total_pages ? 'disabled' : '' ?>>Siguiente</a></li>
        </ul>
    </nav>
</main>
...
document.getElementById('searchButton').addEventListener('click', function() {
    ...
    window.location.href = '?page=catalogoProducto&search_query=' + encodeURIComponent(searchQuery) + '&categoriaProducto_idCategoriaProducto=' + categoriaId + '&marca_idMarca=' + marcaId + '&current_page=1';
});
